    ...
    while (visit_q.size() > 0) {
        while (visit_q.size() > 0) {
            ...
            if (reached_final_state != nullptr) {
                ...
                parent_binding->add(start, reached_final_state->bfs_id);
                return true;
            } else {
                visit_q.pop();
            }
...
boost::unordered_flat_set<ObjectId, objectid_hash> set_difference(
    const boost::unordered_node_set<ObjectId, objectid_hash>& s1,
...
const MultiSourceSearchState*
    BFSMultipleStarts<MULTIPLE_FINAL>::expand_neighbors(const SearchNodeId& current_state_id)
{
    ...
    while (current_transition < automaton.from_to_connections[current_state_id.first].size()) {
        ...
        while (iter->next()) {
            ...
            boost::unordered_flat_set<ObjectId, objectid_hash> difference = set_difference(
                bfses_to_be_visited_by_current_state,
                    ...
                    for (const auto& bfs_to_visit_new_state_next : difference) {
                        if (MULTIPLE_FINAL) {
                            auto start_end_destination_node_ids_pair = std::make_pair(
                                bfs_to_visit_new_state_next.id,
                                new_node_id.second.id
                            );
                            auto node_reached_final = reached_final.find(start_end_destination_node_ids_pair);
...
template<bool MULTIPLE_FINAL>
void BFSMultipleStarts<MULTIPLE_FINAL>::print(std::ostream& os, int indent, bool stats) const
{
    if (stats) {
    ...
    os << std::string(indent, ' ') << "Paths::Any::BFSMultipleStarts(path_var: " << path_var
       << ", start: " << start << ", end: " << end << ")\n";
